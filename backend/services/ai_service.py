    ...
    async def translate_nl_to_sql(
        ...
        try:
            ...
            system_message = (
                ...
                "- 'remove', 'delete', 'get rid of', 'clear' → DELETE or TRUNCATE or DROP (see below)\n"
                "- 'remove all tables', 'delete all tables', 'drop all tables', 'remove every table' → DROP TABLE for each table\n"
                "- 'remove all', 'delete everything', 'clear all', 'wipe', 'empty' (for a specific table) → TRUNCATE TABLE\n"
                "- 'remove some', 'delete where', 'remove X rows' → DELETE\n"
                "- 'drop table', 'delete table', 'remove table' → DROP TABLE\n"
                "- 'remove all data', 'clear table data' (not removing the table itself) → TRUNCATE TABLE\n\n"
                ...
                "6. REMOVE DATA OR TABLES:\n"
                "   - 'remove all tables', 'delete all tables', 'drop all tables' → Generate DROP TABLE for each table\n"
                "   - 'remove all', 'delete everything', 'clear table', 'wipe' (for a specific table) → TRUNCATE TABLE\n"
                "   - 'remove all data', 'clear table data' (not removing table structure) → TRUNCATE TABLE\n"
                ...
                "   - 'drop table X', 'delete table X', 'remove table X' → DROP TABLE X\n\n"
                ...
                "   - If user says 'remove all tables' or 'delete all tables', generate DROP TABLE for EACH table in the schema\n"
                "     Example: If schema has tables [employees, products, orders], generate:\n"
                "     DROP TABLE employees CASCADE; DROP TABLE products CASCADE; DROP TABLE orders CASCADE;\n\n"
                "8. GENERAL RULES:\n"
